        ...
        @elseif($options->type == 'hasMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $comment_selected_values = $model::where($options->column, '=', $relationshipData->{$options->key})->get()->all();
                @endphp
                ...
                @else
                    ...
                    @else
                        <div class="chat_window">
                            <ul class="messages">
                                @foreach($comment_selected_values as $key => $selected_value)

                                    @php
                                        $p_author = $selected_value->projectId()->first()->author_id;
                                        $c_author = $selected_value->author_id;
                                    @endphp

                                    @if($p_author == $c_author)
                                    <li class="message left appeared">
                                    @else
                                    <li class="message right appeared">
                                    @endif
                                        <div class="avatar">
                                        <img src="@if( !filter_var($selected_value->authorId()->first()->avatar, FILTER_VALIDATE_URL)){{ Voyager::image( $selected_value->authorId()->first()->avatar ) }}@else{{ $selected_value->authorId()->first()->avatar }}@endif" class="avatar_inner" alt="{{ $selected_value->authorId()->first()->name }} avatar">
                                        </div>
                                        <div class="text_wrapper">
                                            <div class="name">
                                                {{ $selected_value->authorId()->first()->name }}
                                                @if($selected_value->authorId()->first()->role()->first()->name != 'user')
                                                - ({{ $selected_value->authorId()->first()->role()->first()->display_name }})
                                                @endif
                                            </div>
                                            <div class="text">{!! $selected_value->comment_body !!}</div>
                                            <div class="date"><small>Created on {{ \Carbon\Carbon::parse($selected_value->created_at)->format('d/m/Y') }}</small></div>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
